...
class TokenAcquirer(object):
    ...
    RE_TKK = re.compile(r"(TKK=.)(\d{6}\.[0-9]+)(.;)", re.DOTALL)
    RE_TKK_EVAL = re.compile(r'TKK=eval\(\'\(\(function\(\)\{(.+?)\}\)\(\)\)\'\);',
                             re.DOTALL)
    ...
    def _update(self):
        ...
        try:
            ...
            result = re.search(self.RE_TKK, r.text).group(2)
        except AttributeError:
            # this will be the same as python code after stripping out a reserved word 'var'
            code = unicode(self.RE_TKK_EVAL.search(r.text).group(1)).replace('var ', '')
